...
\begin{demo}{} Se define la clase  $\mathscr{M}'=\{ H: f^{-1}(H)\;\mbox{ es medible}\}$ y  se prueba que 
$\mathscr{M}^{'}$ es $\sigma$-\'algebra de $\mathbb{R}$ que contiene a los abiertos. Esto implica que  $\mathscr{B}(\rr^n) \subset \mathscr{M}$, que es lo que hay que demostrar.
\end{demo}
...
$$
\Leftrightarrow \text { Si } a \in \mathbb{R} \forall=[a,+\infty]=[a,+\infty) \cup<\infty
$$

$[a,+\infty)$ es corrodo de $\mathbb{R}$ eutorcos $[a,+\infty)$ es Borelianos, Entowos $[a,+\infty]$ es Borclicionode $\overline{\mathbb{R}}$

$$
M_e \Rightarrow f^{-1}(川)=f^{-1}([\varepsilon,+\infty])=\{f \rightrightarrows a\}
$$
$$
\Rightarrow \quad f^{-1}(\{\infty\})=f^{-1}\left(\bigcap_{m=1}^{\infty}[m,+\infty]\right)=\bigcap_{n=1}^{\infty} f^{-1}([n,+\infty])
$$


Cach $f^{-1}([m,+\infty])=\left\langle f \geqslant n\left\{\in M_e\right.\right.$
Comolosióm $\left.f^{-1}(\varepsilon \infty)\right) \in M_{\varepsilon}$. Aucúlogamente

$$
\left.f^{-1}(\varepsilon-\infty\}\right) \in \mu_e
$$

sea $H \in B(\overline{\mathbb{R}})=\{B \mid B$ brelianos $\overline{\mathbb{R}}\}$
