...
\begin{ejercicio}{}
$\|f\|_{\infty}$ es cota esencial de $f$ y de hecho es la m\'as chica de las cotas.
\end{ejercicio}
...
\begin{ejemplo}{}
Las funciones escalonadas no son densas  en $L^{\infty}(E)$. Por ejemplo, si $\varphi$ es escalera en $\rr$ se tiene  $\|1-\varphi\|_{\infty}\geq 1$. 
...
\begin{demo}
Sea $m_k \in M$ tal que  
\[d_k:= \|x-m_k\| \leq  d(x,H)+\frac{1}{k}=:d+\frac{1}{k}. \]
...
\end{split}
\]
A partir de aqu\'i, se deduce que $(x-m,h)\leq 0$. Luego, como \'esto vale para $-h$ se concluye $2)$.

$2) \Rightarrow 1)$ Sea $h\in M$. Luego
\[
\|x-m\|^2=(x-m,x-m)=(x-m,x-h)\leq \|x-m\| \|x-h\|.
\]
Dividiendo por $\|x-m\|$ se deduce el enunciado
...
Como $L^2 \cap L^1$ es denso en $L^1$ tenemos $\ell=\ell_g$ en $L^1$.
...
Supondremos que $\|f\|_p,\|g\|_p>0$. Como $t^p$ es convexa para $1\leq p <\infty$ tenemos 
\[
...
\begin{enumerate}
    ...
    Luego, 
    \[
    \sup\limits_{\|g\|_q\leq 1} \int_E fg\,dx \geq 
    \int_E fg_k\,dx \geq \int_E f_k g_k \,dx=\|f_k\|_p \nearrow\infty.
    \]
        \end{enumerate}
\end{demo}
