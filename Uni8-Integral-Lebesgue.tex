...
\begin{itemize}
    ...
    \[
    \int_E c\varphi = \sum\limits_{i=1}^N c\alpha_i m(E_i)=c\int_E \varphi.
    \]
    \end{demo}
    ...
    El supremo se toma sobre todas las funciones simples $\varphi$.
    
    \end{teorema}
    
    \begin{demo}
    Sean $E=E_1\cup E_2\cup\ldots \cup E_N$ y $\alpha_i=\inf\limits_{E_i} f$. Para $k\in \nn$ definamos 
...
Poniendo 
\[
E_k=\{x\in E: f_k(x)> \varphi (x)-\epsilon \}.
\]
Por hip\'otesis
\[
     ...
     \]
          ...
          \]
                   ...
                   % \\
                    ...
                    \begin{demo}
                    A partir de  que $f^+\leq f_1$ y $f^-\leq f_2$ se deduce que $f$ es integrable.  
                    \\
                ...
                \end{enumerate}
                    ...
                    \begin{itemize}
                        ...
                        \sum\limits_{i=1}^N \alpha_i \int f_i= \int f.
                        \]
                    ...
                    Una funci\'on $\mu:\Sigma \to \rr$ se llama \emph{medida con signo}\index{medida con signo} si
                    \begin{enumerate}
                        \item $\mu(\emptyset)=0,$
                    ...
                    Para una medida con signo valen los teoremas de convergencia mon\'otona de conjuntos, se puede definir una integral y \emph{casi todo} lo visto para la medida de Lebesgue es v\'alido. Una propiedad que no siempre es cierta es la invariancia por traslaciones.
                    
                    Ahora bien,  $\Phi$ es una medida con signo. 
                    La pregunta que surge es ?`ser\'a toda medida con signo sobre
                    ...
                    \begin{demo}      
        Si $f$ es integrable Riemann, entonces para todo $k\in\mathbb{N}$  existe una partición $a=x_0^k<x_1^k<x_2^k<\ldots<x_N^k=b$ tal que 
        
        $$S-s=\sum_{i=1}^N(M_i^k-m_i^k)(x_{i}^k-x_{i-1}^k)<\frac{1}{k}.$$
        
        Consideramos   las funciones escaleras $\varphi_k$ y $\psi_k$ definidas por
        
       \[
        \varphi_k(x)=\sum\limits_{i=1}^N m_i^k \chi_{J^k_i} \;\;\mbox{ y }\;\;
        \psi_k(x)=\sum\limits_{i=1}^N M_i^k \chi_{J_i^k} 
        \]
        donde 
        $J_i^k=[x^k_{i-1},x^k_i]$. Entonces $\varphi_k \leq f \leq \psi_k$ en c.t.p. de $[a,b]$ y 
        \[
        \int_a^b \psi_k -\varphi_k <\frac{1}{k}.
        \]
        Sean  $g=\sup\varphi_k$ y $h=\inf \psi_k$. Las funciones       $g$ y $h$ son borelianas y $g\leq f\leq h$. Entonces 
        
        
        $$ \int_a^b \varphi_k \leq \int_a^b g \leq \int_a^b f\leq \int_a^b h   \leq \int_a^b \psi_k $$
       De allí se concluye
       
        $$\int_a^b h- g   \leq \int_a^b \psi_k -\int_a^b \varphi_k \leq \frac{1}{k}. $$
        En con secuencia, puesto que $h-g\geq 0$ c.t.p., se deduce que $h=g$ en c.t.p y por ende $f=h=g$ en c.t.p. Esto último implica que $f$ es boreliana y de allí medible.
        
        Por otro lado, por la monotonían de la integral de Lebesgue
        \[
        \int_a^b \varphi_k \leq \int_a^b f \leq \int_a^b \psi_k.
        \]
        Y por la definición de integral de Riemann
        \[
        \int_a^b \varphi_k \leq\;  \R \,\int_a^b f \leq\;  \int_a^b \psi_k,
        \]
        de donde
        \[
        \left|
       ...
       \; \mbox{ y }\;
        ...
        \begin{enumerate}
            \item $E_x$ es medible de $\rr^{m}$ en c.t.p. $x \in \rr^n$;
        ...
        \begin{demo}
        \emph{Primer Paso)} Si $E$ es un rectángulo de $\rr^{n+m}$, supongamos que $E=I\times J$ con $I$ rectángulo de $\rr^n$, $J$ rectángulo de $\rr^m$, entonces $E_x=J$ $\forall x \in I$ y $E_x=\emptyset$ si $x \notin I$.
        As\'i, $E_x$ es conjunto medible $\forall x \in I$, $m(E_x)=\chi_I m(J)$ es funci\'on medible  en $x$ y 
        \[
        \int m(E_x)\,dx=m(J)m(I)=m(E).
        \]
        \emph{Segundo Paso)} Si $E$ es abierto, entonces $E_x=\bigcup\limits_{k=1}^{\infty} (I_k)_x$ con $I_k$ rectángulos mutuamente disjuntos y donde $E_x=\bigcup\limits_{k=1}^{\infty} \left(I_k\right)_x$. 
