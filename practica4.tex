...
\begin{ejer}{} \emph{(Ejemplo de conjunto medible no boreliano)}. Considérese la función $\psi$ definida por la fórmula $\psi(x)=x+\varphi(x)$, donde $\varphi$ es la función singular de Cantor.
Mostrar que:

\begin{enumerate}[label=\alph*)]
\item $\psi$ es estrictamente creciente y continua; su inversa $\psi^{-1}$ tiene las mismas propiedades.
\item para cada conjunto boreliano $B \subset R, \psi(B)$ y $\psi^{-1}(B)$ son borelianos.
\item $\psi([0,1])=[0,2]$
\item poniendo $G=[0,1]-P$ (complemento del conjunto ternario relativo a $[0,1])$ se tiene $m(\psi(G))=m(G)=1$.
\item $m(\psi(P))=1$.
\item existe un conjunto no medible $V \subset \psi(P)$.
\item $\psi^{-1}(V)$ es medible Lebesgue pero no es medible Borel.
 \end{enumerate}
\end{ejer}
