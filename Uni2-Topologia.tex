...
$(X,d)$ es un \index{espacio!métrico|textbf}\emph{espacio métrico}.
\end{definicion}
\marginnote{
\begin{center}
...
La desigualdad iii) en la definición anterior se denomina\index{desigualdad!triángular|textbf}
...
como \emph{métrica euclidea}\index{métrica!euclidea|textbf} y es la métrica con la que estamos más familiarizados.
\end{ejemplo}
...
\begin{ejemplo}{ejem,caballo}[Métrica caballo ajedrez]\index{métrica!caballo ajedrez} Sobre el conjunto $\mathbb{Z}^2$ definimos la métrica $d$ por $d((x_0,y_0),(x_1,y_1))$  como la menor cantidad de movidas necesarias dar con un caballo de ajedrez para ir desde $(x_0,y_0)$ a $(x_1,y_1)$.  
\end{ejemplo}



\begin{ejemplo}{ejem,geodesia}[Métrica de las geodesias] Sobre el conjunto 
...
\]
donde $\langle P, Q \rangle$ denota el producto interno entre $P$ y $Q$ y para la función $\arccos$ tomamos la rama con valores en el intervalo $[0,\pi]$. Entonces $d_g$ es una métrica, que se denomina \emph{métrica geodésica}\index{métrica!geodésica}. La afirmación de que $d_g$ es métrica amerita justificación.
...
% Si los conjuntos de vectores $\{P,Q\}$ y $\{P,R\}$ fueran linealmente independientes ambos, deduciríamos que existirían $\lambda,\mu\in\rr$ con $Q=\lambda P$ y $R=\mu P$. Luego tambián $Q=\lambda/\mu P$. Razonando como antes, puesto que los vectores tienen norma 1 concluímos que $\lambda=\pm 1=\mu$. De esto deducimos o que $P=Q=R$ o que hay dos iguales y uno opuesto, por ejemplo $P=Q$ y $P=-R$. Encualquiera de estas situaciones es fácil verificar la desigualdad triangular y lo dejamos como ejercicio. Entonces podemos suponer que alguno dfe los conjuntos $\{P,Q\}$ o $\{P,R\}$ son linealmente independientes. Ahora podemos suponer que los dos lo son, porque de no ser así, supongamos  $\{P,Q\}$  linealmente independiente y  $\{P,R\}$ linealmente dependiente. 
...
concepto de \emph{bola}, \emph{esfera} y \emph{diámetro}.\index{bola|textbf} \index{esfera|textbf}  \index{diámetro|textbf}
...
\begin{definicion}{def:cont_unif} Sea $f$ una función entre dos e.m. $(X,d)$ e $(Y,d')$.
...
\begin{definicion}{} Una \index{Sucesión|textbf}\emph{sucesión} en un e.m. $(X,d)$ es una
función $f:\nn\rightarrow X$.
\end{definicion}
...
\]
Entonces existe un $\delta>0$ tal que si $A\subset X$ es un conjunto con $\operatorname{diam}(A)<\delta$ entonces existe $i\in\mathscr{I}$ tal que $A\subset G_i$. El número $\delta$ se suele llama  \emph{número de Lebesgue} \index{número! Lebesgue} del cubrimiento.
...
\begin{ejercicio}{ejer:comp=>alcanzamax} Demostrar que si $f:(X,d)\to (Y,d')$ es continua
...
\begin{ejercicio}{ej:cont=>unif_cont} Demostrar que si $f:X\to Y$ es continua y $X$ es compacto entonces $f$ es uniformemente continua.
\end{ejercicio}
